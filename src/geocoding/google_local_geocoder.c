...
#include "geocoder.h"
#include "google_local_geocoder.h"
...
#define GOOGLE_LOCAL_QUERY_URL "http://ajax.googleapis.com/ajax/services/search/local?q=%s%%20loc:%f,%f&start=0&rsz=large&v=1.0"

char* google_local_prepare_url(char* encodedQuery) {
	char* url;
	if (asprintf(&url, GOOGLE_LOCAL_QUERY_URL, encodedQuery, canvas.center.latitude, canvas.center.longitude) < 0) {
		...
		if (json_results != NULL) {
			...
			if (length > 0) {
				for (i = 0; i < length; i++) {
					...
					result -> name = json_object_get_string(json_object_object_get(json_result, KEY_TITLE_NO_FORMATTING));
